...
async function insertIntoDatabaseInOrder(
  slot: TimeSlot,
  email: string,
  userId: string,
...
async function insertTimeSlotsInOrder(
  slots: TimeSlot[],
  email: string,
  userId: string,
  availabilityPosting: any,
  ...
  for (const slot of slots) {
    const createdSlot = await insertIntoDatabaseInOrder(
      slot,
      email,
      userId,
      availabilityPosting,
    ...
    createdTimeSlots.push(createdSlot);
  }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const createdTimeSlots = await insertTimeSlotsInOrder(
      timeSlots,
      email,
      userId,
      availabilityPosting,
      ...
      try {
        const message = await client.messages.create({
          body: `Hello from Chat Signal! ${currentUser.name} is about to be available for a phone call from ${convertToUserTimezone(
            ...
            timezone
          )} for time slots of ${duration} minutes - do you want to chat with them? Claim a slot below:\n\n${slotStr}`,
